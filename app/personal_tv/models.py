...
class ProgrammingPreferences(db.Model):
    ...
    deprioritized_creators: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
...
class TVSessionItem(db.Model):
    ...
    playhead_seconds: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
