...
def test_personal_tv_builds_a_session_from_youtube_groups(page, live_app, app):
    ...
    expect(page.locator("fieldset").filter(has_text="Collections")).to_be_visible()
    expect(page.get_by_role("button", name="Deepen selected collections")).to_be_visible()
    expect(page.get_by_text("my favoret")).to_be_visible()
    ...
    page.get_by_role("button", name="Science & Knowledge2 cached").click()
    page.get_by_role("button", name="Build My TV").click()
    expect(page.get_by_role("heading", name="Review the lineup")).to_be_visible()
    expect(page.get_by_text("Science programme one")).to_be_visible()
