...
| Reusable rails | Many horizontal poster sections | `MediaSection` | Grid/list and compact recommendation, not reusable rails | Config-driven `MediaRail` with skeleton/loading contract | BUILD_NEW |
...
| Skeletons | Implied loading polish | `Skeletons` components | Existing structural server render and local loaders | Hero/card/rail/detail/episode skeleton family | BUILD_NEW |
| Toasts | Interaction feedback style | `ToastContainer` | Existing shared feedback patterns | Reusable Dragon toast only for reversible confirmations | ADAPT_IDEA |
